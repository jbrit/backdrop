...
const { paystackAccountName } = require("../utils");
  ...
  const req = https.request(options, (res) => {
    ...
    res.on("end", async () => {
      ...
      const [user, created] = await User.User.findOrCreate({
        ...
        defaults: {
          user_account_name: paystackAccountName.toLowerCase(),
          is_verified: false,
        },
      });
      ...
      if (!created && user.is_verified === false) {
        ...
        const isMatch =
          paystackAccountName.toLowerCase() === user_account_name.toLowerCase();
        if (isMatch) {
          ...
          const user = await User.User.findOrCreate({
            ...
            defaults: {
              user_account_name: user_account_name.toLowerCase(),
              is_verified: true,
            },
          });
          return user;
